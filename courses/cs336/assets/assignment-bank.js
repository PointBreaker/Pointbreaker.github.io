(function () {
  ...
  window.CS336AssignmentBank = {
    '01': {
      ...
      archiveHref: 'archive-2025-ass01-basics.html',
      changes: ['训练与 leaderboard 的硬件假设更新为 B200。', 'Tokenizer、special-token 与 architecture accounting 约束得到澄清。', 'AdamW update order、tests 与 tensor typing 以当前 repository 为准。'],
    ...
    '02': {
      ...
      archiveHref: 'archive-2025-ass02-systems.html',
      changes: ['新增 activation checkpointing、详细 Nsight 分析与 FSDP。', 'FlashAttention backward 改为分离 dK/dV 与 dQ 的 two-pass 路线。', 'Bucketed DDP 与过量 multi-GPU sweeps 不再是 required route。'],
    ...
    '03': {
      ...
      archiveHref: 'archive-2025-ass03-scaling.html',
      changes: ['模型训练从本地/offline workflow 改为 hosted online training API。', 'Run planning 必须同时记录 budget、失败任务、fit region 与 holdout。'],
    ...
    '04': {
      ...
      archiveHref: 'archive-2025-ass04-data.html',
      changes: ['Active corpus 改为由 2,500 个 raw WET files 派生的 English-only workflow。', '最终训练 recipe 更新为 8×B200、16,384 steps、约 8.6B processed tokens。'],
    ...
    '05': {
      ...
      archiveHref: 'archive-2025-ass05-alignment.html',
      changes: ['主实验从 Qwen/MATH 迁移到 OLMo-2-1B/GSM8K。', '加入 zero/few/r1-zero baselines、Dr. GRPO、RFT、MaxRL 与 GSPO。', '关键实验要求四个随机种子；安全 SFT/DPO 保持为独立 optional supplement。'],
