(() => {
  ...
  function addText(parent: SVGElement, text: string, x: number, y: number, className: string, anchor = 'middle'): SVGTextElement {
  ...
  class TensorParallelVisualizer {
    ...
    private stepList!: HTMLOListElement;
    private title!: HTMLElement;
    private status!: HTMLElement;
    private previousButton!: HTMLButtonElement;
    private nextButton!: HTMLButtonElement;
    private operation!: HTMLElement;
    private shape!: HTMLElement;
    private communication!: HTMLElement;
    ...
    private requireElement<T extends Element>(selector: string): T {
      ...
      if (!element) throw new Error(`Tensor parallel visualizer is missing ${selector}`);
      return element;
    }
    ...
    private buildSvg(): SVGSVGElement {
      ...
      xPositions.forEach((x, index) => {
        ...
        for (let cell = 0; cell < 6; cell += 1) weight.append(svgElement('rect', { x: x + 22 + cell * 24, y: 190, width: 23, height: 22, class: 'tp-svg-cell' }));
        group.append(weight);
        ...
        for (let cell = 0; cell < 6; cell += 1) output.append(svgElement('rect', { x: x + 22 + cell * 24, y: 304, width: 23, height: 18, class: 'tp-svg-cell' }));
        group.append(output);
        svg.append(group);
      });
    ...
    private handleClick(event: Event): void {
      ...
      if (!button) return;
      if (button.dataset.step !== undefined) this.setStep(Number(button.dataset.step));
      if (button.dataset.action === 'previous') this.setStep(this.currentStep - 1);
      if (button.dataset.action === 'next') this.setStep(this.currentStep + 1);
      if (button.dataset.action === 'reset') this.setStep(0);
    }

    private handleKeydown(event: KeyboardEvent): void {
      if (event.target instanceof HTMLButtonElement) return;
      if (event.key === 'ArrowLeft') { event.preventDefault(); this.setStep(this.currentStep - 1); }
      if (event.key === 'ArrowRight') { event.preventDefault(); this.setStep(this.currentStep + 1); }
      if (event.key === 'Home') { event.preventDefault(); this.setStep(0); }
      if (event.key === 'End') { event.preventDefault(); this.setStep(steps.length - 1); }
    }

    private setStep(nextStep: number): void {
    ...
    private render(): void {
      ...
      this.requireElement<HTMLElement>('.tp-step-count').textContent = `Step ${this.currentStep + 1} of ${steps.length}`;
      ...
      if (description) description.textContent = step.ariaLabel;
  ...
  const mount = (): void => {
    document.querySelectorAll<HTMLElement>('[data-tp-visualizer]').forEach(root => {
      if (!root.dataset.ready) new TensorParallelVisualizer(root);
    });
  };
