(() => {
  ...
  type Stage = {
    ...
    operation: (parameters: Parameters) => string;
    output: (parameters: Parameters) => string;
    changed: (parameters: Parameters) => string;
    why: string;
    activeNodes: string[];
    changedAxes: number[];
  };
  ...
  type DiagramNode = {
    ...
    y: number;
    width: number;
    shape: (parameters: Parameters) => string;
    stage: number;
  };

  const DEFAULTS: Parameters = { sequenceLength: 8, heads: 8, model: 512 };
  const TOKENS = ['The', 'quick', 'brown', 'fox', 'jumped', 'over', 'the', 'log', 'then', 'looked', 'back', 'at', 'the', 'quiet', 'green', 'field'];

  const dHead = (parameters: Parameters): number => parameters.model / parameters.heads;
  const dFf = (parameters: Parameters): number => parameters.model * 4;
  const modelShape = (parameters: Parameters): string => `[B, ${parameters.sequenceLength}, ${parameters.model}]`;
  const headShape = (parameters: Parameters): string => `[B, ${parameters.heads}, ${parameters.sequenceLength}, ${dHead(parameters)}]`;
  const scoreShape = (parameters: Parameters): string => `[B, ${parameters.heads}, ${parameters.sequenceLength}, ${parameters.sequenceLength}]`;
  ...
  class TransformerRefresher {
    ...
    private selectedToken = 3;
    private selectedHead = 0;
    private focusedTensor = 'residual-start';
    ...
    private requireElement<T extends Element>(selector: string): T {
      ...
      if (!element) throw new Error(`Transformer refresher is missing ${selector}`);
      return element;
    }
    ...
    private handleClick(event: Event): void {
      ...
      const tensor = target.closest<SVGGElement>('[data-tensor]');
      ...
      if (!button) return;
      if (button.dataset.stage !== undefined) this.setStage(Number(button.dataset.stage));
      if (button.dataset.action === 'previous') this.setStage(this.currentStage - 1);
      if (button.dataset.action === 'next') this.setStage(this.currentStage + 1);
      if (button.dataset.action === 'play') this.togglePlay();
    ...
    private handleChange(event: Event): void {
      const select = (event.target as Element).closest<HTMLSelectElement>('select[data-parameter]');
      if (!select) return;
      const value = Number(select.value);
      if (select.dataset.parameter === 'sequenceLength') this.parameters.sequenceLength = value as Parameters['sequenceLength'];
      if (select.dataset.parameter === 'heads') this.parameters.heads = value as Parameters['heads'];
      if (select.dataset.parameter === 'model') this.parameters.model = value as Parameters['model'];
    ...
    private handleKeydown(event: KeyboardEvent): void {
      if (event.target instanceof HTMLButtonElement || event.target instanceof HTMLSelectElement || event.target instanceof HTMLElement && event.target.closest('summary')) return;
      if (event.key === 'ArrowLeft') { event.preventDefault(); this.setStage(this.currentStage - 1); }
      if (event.key === 'ArrowRight') { event.preventDefault(); this.setStage(this.currentStage + 1); }
      if (event.key === 'Home') { event.preventDefault(); this.setStage(0); }
      if (event.key === 'End') { event.preventDefault(); this.setStage(stages.length - 1); }
      if (event.key === ' ' && event.target === this.root) { event.preventDefault(); this.togglePlay(); }
    }

    private setStage(nextStage: number): void {
      this.currentStage = Math.max(0, Math.min(stages.length - 1, nextStage));
      if (this.currentStage === stages.length - 1) this.stopPlaying();
      this.render();
    }
    ...
    private togglePlay(): void {
      ...
      if (this.currentStage === stages.length - 1) this.currentStage = 0;
    ...
    private reset(): void {
      ...
      this.root.querySelectorAll<HTMLSelectElement>('select[data-parameter]').forEach(select => {
        const key = select.dataset.parameter as keyof Parameters;
    ...
    private render(): void {
      ...
      this.requireElement<HTMLElement>('.tr-steps').innerHTML = stages.map((item, index) => `
      ...
      this.requireElement<HTMLElement>('.tr-step-count').textContent = `Step ${this.currentStage + 1} of ${stages.length}`;
      this.requireElement<HTMLElement>('#tr-stage-title').textContent = stage.title;
      this.requireElement<HTMLElement>('[data-field="input"]').textContent = stage.input(this.parameters);
      this.requireElement<HTMLElement>('[data-field="operation"]').textContent = stage.operation(this.parameters);
      this.requireElement<HTMLElement>('[data-field="output"]').textContent = stage.output(this.parameters);
      this.requireElement<HTMLElement>('[data-field="changed"]').textContent = stage.changed(this.parameters);
      this.requireElement<HTMLElement>('[data-field="why"]').textContent = stage.why;
    ...
    private renderShapeEquation(): void {
      ...
      this.requireElement<HTMLElement>('.tr-shape-equation').innerHTML = `
    ...
    private nodeMarkup(node: DiagramNode): string {
      const stage = stages[this.currentStage];
    ...
    private displayShape(node: DiagramNode): string {
    ...
    private renderDiagram(): void {
      const diagram = this.requireElement<HTMLElement>('.tr-diagram-wrap');
      ...
      this.requireElement<HTMLElement>('.tr-focus-readout').textContent = `Focused tensor · ${focused.label}: ${this.displayShape(focused)}`;

      this.requireElement<HTMLElement>('.tr-mobile-flow').innerHTML = diagramNodes
    ...
    private attentionWeights(): number[] {
      ...
      const raw = Array.from({ length }, (_, key) => {
        ...
        if (mode === 0) return Math.exp(-distance * 0.65);
        if (mode === 1) return 0.25 + ((key + this.selectedHead) % 3 === 0 ? 1.45 : 0.12);
        if (mode === 2) return key === 0 ? 1.8 : 0.35 + 0.2 * Math.cos((key + 1) * (this.selectedHead + 1));
    ...
    private renderAttentionExplorer(): void {
      const explorer = this.requireElement<HTMLElement>('.tr-attention-explorer');
    ...
    private renderMixingComparison(): void {
      const comparison = this.requireElement<HTMLElement>('.tr-mixing-comparison');
    ...
    private renderControls(): void {
      const previous = this.requireElement<HTMLButtonElement>('[data-action="previous"]');
      const next = this.requireElement<HTMLButtonElement>('[data-action="next"]');
      const play = this.requireElement<HTMLButtonElement>('[data-action="play"]');
      ...
      this.requireElement<HTMLElement>('.tr-status').textContent = `Step ${this.currentStage + 1} / ${stages.length}`;
    }
  ...
  const mount = (): void => {
    document.querySelectorAll<HTMLElement>('[data-transformer-refresher]').forEach(root => {
      if (!root.dataset.ready) new TransformerRefresher(root);
    });
  };
