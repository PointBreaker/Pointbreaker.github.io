#!/usr/bin/env python3
"""Run a bounded Claude Code extraction and capture verified structured output."""
...
def verified_model_usage(
    payload: dict[str, Any],
    *,
    expected_canonical_models: set[str],
    expected_provider: str | None,
    expected_context_window: int | None,
    ...
    if not canonical:
        raise ValueError("Claude CLI omitted the canonical model")
    if provider in (None, ""):
        raise ValueError("Claude CLI omitted the model provider")
    if not isinstance(context_window, int) or context_window <= 0:
        raise ValueError(f"Claude CLI returned an invalid context window: {context_window!r}")
    if expected_canonical_models and canonical not in expected_canonical_models:
        raise ValueError(f"unexpected canonical model: {canonical!r}")
    if expected_provider is not None and provider != expected_provider:
        raise ValueError(f"unexpected provider: {provider!r}")
    if expected_context_window is not None and context_window != expected_context_window:
...
def main() -> int:
    ...
    parser.add_argument("--tools", choices=("Read", "none"), default="Read")
    parser.add_argument(
        "--no-tools",
        action="store_true",
        help="Disable tools for prompts that already embed all required source text.",
    )
    parser.add_argument("--autocompact")
    parser.add_argument("--expected-canonical-model", action="append", default=[])
    parser.add_argument("--expected-provider")
    parser.add_argument("--expected-context-window", type=int)
    ...
    tools = "" if args.no_tools or args.tools == "none" else args.tools
    if tools not in SAFE_TOOL_ALLOWLISTS:
        parser.error(f"unsafe tool allowlist: {tools!r}")
    ...
    expected_models = {value.lower() for value in args.expected_canonical_model}
    ...
    probe_label, probe_usage = verified_model_usage(
        probe,
        expected_canonical_models=expected_models,
        expected_provider=args.expected_provider,
        expected_context_window=args.expected_context_window,
    )
    if structured_output(probe) != {"ok": True}:
        raise ValueError("Claude preflight returned unexpected structured output")
    ...
    extraction_label, extraction_usage = verified_model_usage(
        extraction,
        expected_canonical_models=expected_models,
        expected_provider=args.expected_provider,
        expected_context_window=args.expected_context_window,
    )
    identity_fields = ("canonicalModel", "provider", "contextWindow")
    if extraction_label != probe_label or any(
        extraction_usage.get(field) != probe_usage.get(field) for field in identity_fields
    ):
        raise ValueError("model identity changed after preflight")
    ...
    run_record = {
        ...
        "expectedCanonicalModels": sorted(expected_models),
        "expectedProvider": args.expected_provider,
        "expectedContextWindow": args.expected_context_window,
        "autocompact": args.autocompact,
        "toolMode": "none" if not tools else "read-only",
        ...
        "workdir": str(workdir),
        "readDirectories": [str(path) for path in add_dirs] if tools else [],
