...
Serve the repository locally. Inspect the homepage catalog entry, Dashboard, at least two lectures, every page type, representative math and code, quiz feedback, the GitHub Discussion drawer, mobile width, links, and browser console. Confirm that no KaTeX node uses the error fallback color, `window.__mathRenderErrors` is empty, every `pre` owns a `code.language-*` child, and non-plaintext examples contain Prism tokens. On courses with exams, verify stage boundaries, `Lecture → HW → Exam` order, non-repeated lectures, separate HW/Exam counts and filters, and every work-item link. On desktop, verify that opening Discussion reserves space instead of covering the lesson. For every table-bearing page type, verify that `.pb-table-scroll` has non-zero intrinsic width on desktop, stays within its parent, and becomes a full-width horizontal scroller on mobile; never combine `width: fit-content` with inline-size or size containment. Treat placeholders, math errors, unformatted code, clipped layouts, collapsed or trailing-blank tables, missing shared assets, broken navigation, invalid dependency stages, and overlapping discussion UI as blockers.
